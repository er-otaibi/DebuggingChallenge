...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        submitBtn.setOnClickListener {
            if (usernameAccepted(userName.text.toString()) &&
                passwordAccepted(password.text.toString()) &&
                identicalPassword(password2.text.toString())
            ) {
    ...
    fun identicalPassword(text: String): Boolean {
    ...
    private fun passwordAccepted(text: String): Boolean {
        if (text.length >= 8) {
            ...
            Toast.makeText(this, "The password must contain an uppercase letter", Toast.LENGTH_LONG)
                .show()
            return false
        }
        Toast.makeText(this, "The password must be at least 8 characters long", Toast.LENGTH_LONG)
            .show()
        return false
    ...
    private fun hasUpper(text: String): Boolean{
        ...
        while (letter <= 'Z') {
            ...
            if(text.contains(letter, ignoreCase = false)){
                return true
            }
            ++letter
        }
